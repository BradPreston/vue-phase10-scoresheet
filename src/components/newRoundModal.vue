  ...
  <dialog :open="modalVisible" class="w-full h-screen z-50 fixed top-0 left-0 overflow-y-auto p-4">
    ...
    <form @submit.prevent="handleAddRound" id="addRoundForm" class="flex flex-col justify-center">
      <ul class="flex flex-wrap list-none gap-4 justify-center pl-0">
        <li v-for="player of players" class="flex border-2 border-primary-content bg-secondary flex-col text-center p-4 pt-0 rounded min-w-10">
          <div class="m-0 capitalize font-bold border-b-2 border-b-primary-content p-2">{{ player.name }}</div>
          <div class="m-0 flex flex-col mt-4 gap-4">
            <label class="label cursor-pointer justify-start gap-4">
              <label class="label-text" :for="player.id+'-score'">Score</label>
              <input required :id="player.id+'-score'" type="number" :name="player.name + '-score'" class="input input-bordered" value="0">
            </label>
